...
public class EfCoreAuthenticationStoreTests : EfCoreTestBase
{
    ...
    public async Task Add_And_Get_Should_Work()
    {
        ...
        var store = new EfCoreAuthenticationSecurityStateStore(db, new TenantContext(tenant));
    ...
    public async Task Update_With_RegisterFailure_Should_Increment_Version()
    {
        ...
        {
            var store = new EfCoreAuthenticationSecurityStateStore(db1, new TenantContext(tenant));
            await store.AddAsync(state);
        }

        await using (var db2 = CreateDb(connection))
        {
            var store = new EfCoreAuthenticationSecurityStateStore(db2, new TenantContext(tenant));
        ...
        {
            var store = new EfCoreAuthenticationSecurityStateStore(db3, new TenantContext(tenant));
            var result = await store.GetAsync(userKey, AuthenticationSecurityScope.Account, null);
    ...
    public async Task BeginReset_And_Consume_Should_Work()
    {
        ...
        {
            var store = new EfCoreAuthenticationSecurityStateStore(db4, new TenantContext(tenant));
            var result = await store.GetAsync(userKey, AuthenticationSecurityScope.Account, null);
    ...
    public async Task Should_Not_See_Data_From_Other_Tenant()
    {
        ...
        var store1 = new EfCoreAuthenticationSecurityStateStore(db, new TenantContext(tenant1));
        var store2 = new EfCoreAuthenticationSecurityStateStore(db, new TenantContext(tenant2));
