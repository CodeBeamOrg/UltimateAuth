...
public class EfCoreCredentialStoreTests : EfCoreTestBase
{
    ...
    public async Task Add_And_Get_Should_Work()
    {
        ...
        var store = new EfCorePasswordCredentialStore(db, new TenantContext(tenant));
    ...
    public async Task Save_Should_Increment_Version()
    {
        ...
        {
            var store1 = new EfCorePasswordCredentialStore(db1, new TenantContext(tenant));
            await store1.AddAsync(credential);
        }

        await using (var db2 = CreateDb(connection))
        {
            var store2 = new EfCorePasswordCredentialStore(db2, new TenantContext(tenant));
        ...
        {
            var store3 = new EfCorePasswordCredentialStore(db3, new TenantContext(tenant));
    ...
    public async Task Should_Not_See_Data_From_Other_Tenant()
    {
        ...
        var store1 = new EfCorePasswordCredentialStore(db, new TenantContext(tenant1));
        var store2 = new EfCorePasswordCredentialStore(db, new TenantContext(tenant2));
